...
const CATEGORIES = [
    ...
    },
    {
        ...
        keywords: [
            ...
            'mensuel', 'cotisation', 'renseignement', 'grille',
    ...
    },
    {
        ...
        keywords: [
            ...
            'adhérer', 'rejoindre le club', 'je voudrais m\'inscrire',
            'abonner', "m'abonner",
        ],
    },
    {
    ...
    },
    {
        ...
        keywords: [
            ...
            'résilier', 'resilier', 'résilie', 'annulation', 'administratif', 'contrat',
            'litige', 'réclamation', 'problème', 'erreur', 'prélèvement', 'attestation',
            'gérer mon abonnement', 'gerer mon abonnement',
        ],
    },
];
 ...
 *
 * @param {string} speechResult  Texte issu de la reconnaissance vocale
 * @returns {{ motif: string, label: string, confidence: number }}
 */
function isCancelIntent(speechResult) {
    const text = normalize(speechResult || '');
    return /resil|recil|rezil|desabonn|desinscri|annul.{0,20}abonn|arrete?r?.{0,24}abonn|stopper.{0,20}abonn|mettre fin.{0,20}abonn|ne plus.{0,16}prelev|gerer (mon )?abonnement/.test(text);
}

function classify(speechResult) {
    const text = normalize(speechResult || '');
    if (isCancelIntent(text) || /prelevement|facture|attestation/i.test(text)) {
        return { motif: 'administratif', label: 'Problème administratif / Facture', confidence: 10 };
    }
...
module.exports = { classify, isCancelIntent, CATEGORIES };
