...
/**
 * Envoi de SMS (Twilio SMS) et WhatsApp (Twilio WhatsApp).
 *
 * WhatsApp : WHATSAPP_PROVIDER=twilio (défaut).
 * Optionnel : TWILIO_WHATSAPP_CONTENT_SID pour un modèle Meta (message initié par le club).
 *
...
/**
 * Envoie un message WhatsApp (Twilio par défaut).
 */
function toE164(raw) {
    let n = String(raw || '').replace(/^whatsapp:/i, '').replace(/[^\d+]/g, '');
    if (n.startsWith('00')) n = `+${n.slice(2)}`;
    if (/^0[67]\d{8}$/.test(n)) n = `+33${n.slice(1)}`;
    if (/^33\d{9}$/.test(n)) n = `+${n}`;
    if (n && !n.startsWith('+')) n = `+${n}`;
    return n;
}

/**
 * Envoie un WhatsApp via Twilio (défaut).
 * WHATSAPP_PROVIDER=baileys | gateway  → anciens canaux, uniquement si besoin.
 */
async function sendWhatsApp({ to, body, name, link }) {
    if (DRY_RUN()) {
        log(`💬 [DRY-RUN] WhatsApp → ${to} : "${String(body || '').slice(0, 60)}..."`);
        return { ok: true, sid: 'dry-run' };
    }
    ...
    if (provider === 'gateway' || provider === 'sms') return sendSmsViaGateway({ to, body });
    return sendWhatsAppViaTwilio({ to, body, name, link });
}
 ...
 */
async function sendWhatsAppViaTwilio({ to, body, name, link }) {
    const client  = getClient();
    const fromNum = toE164(process.env.TWILIO_WHATSAPP_NUMBER || '');
    ...
    if (!fromNum) {
        warn('TWILIO_WHATSAPP_NUMBER manquant — WhatsApp non envoyé');
        return { ok: false, error: 'wa_sender_missing' };
    }
    ...
    if (contentSid) {
        ...
        payload.contentVariables = JSON.stringify({
            1: name || 'Boxing Center',
            2: link || process.env.BOXING_WEBSITE || 'https://boxingcenter.fr',
        });
    } else {
        payload.body = body;
    }
    ...
    try {
        ...
        log(`💬 WhatsApp Twilio → ${toNum} [${msg.sid}]`);
        return { ok: true, sid: msg.sid };
    } catch (e) {
        warn(`Erreur WhatsApp Twilio: ${e.message}`);
        return { ok: false, error: e.message };
    }
}
