 ...
 *
 * Public (menus appelants) : coupé.
 * Interne (menu 99) : Twilio, vers la boîte d'entraînement seulement.
 *
...
function normalizeDest(raw) {
    ...
    return String(raw).startsWith('+') ? String(raw) : '';
}

function isTrainDestination(to) {
    const n = normalizeDest(to);
    if (!n) return false;
    const allowed = new Set([TRAIN_INBOX]);
    for (const extra of String(process.env.TRAIN_SMS_TO || '').split(/[,\s]+/)) {
        const p = normalizeDest(extra);
        if (p) allowed.add(p);
    }
...
/**
 * Envoie un SMS. Sans `internal: true`, rien ne part (menus publics).
 * @returns {{ ok: boolean, sid?: string, error?: string, skipped?: boolean }}
 */
async function sendSms({ to, body, internal = false }) {
    if (!internal || !isTrainDestination(to)) {
        log(`📱 SMS public coupé — non envoyé vers ${to}`);
        return { ok: false, error: 'sms_disabled', skipped: true, body };
    }

    if (DRY_RUN()) {
        log(`📱 [DRY-RUN] SMS interne → ${to} : "${String(body || '').slice(0, 60)}..."`);
        return { ok: true, sid: 'dry-run' };
    }

    const client = getClient();
    if (!client) {
        warn('Twilio non configuré — SMS interne non envoyé');
    ...
    try {
        ...
        log(`📱 SMS interne envoyé → ${to} [${msg.sid}]`);
        return { ok: true, sid: msg.sid };
    } catch (e) {
        warn(`Erreur SMS interne: ${e.message}`);
...
module.exports = { sendSms, buildSmsBody, resolveSmsMotif, isConfigured, extractFirstName, capitalizeName, isTrainDestination };
