...
function resolveSmsMotif(motif, extra = {}) {
    ...
    if (/r[ée]sili|arr[eê]ter.{0,24}abonn|stopper.{0,24}abonn|g[ée]rer.{0,16}abonn|facture|pr[ée]l[èe]vement/i.test(last)
        || /r[ée]sili/i.test(blob)) {
        return 'administratif';
    }
    if (/enfant|[ée]ducative|inscri|tarif|offre/i.test(last) && !/planning|horaire|cours/i.test(last)) {
        return 'inscription';
    }
    return SMS_ALIASES[motif] || motif || 'autre';
}
