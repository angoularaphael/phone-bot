...
const SPEECH_RATE = (() => {
    const raw = String(process.env.BOT_SPEECH_RATE || '120%').trim();
...
function toSsml(text) {
    ...
    return `<speak><prosody rate="${SPEECH_RATE}">${withBreaks}</prosody></speak>`;
}
...
function buildGather({ say, action, numDigits = 1, timeout = 10 }) {
    ...
    if (say) gather.say({ voice: VOICE, language: LANGUAGE }, toSsml(say));
...
function buildPlayThenRedirect({ say = null, play = null, action, pause = 2 }) {
    const resp = new VoiceResponse();
    if (say) resp.say({ voice: VOICE, language: LANGUAGE }, toSsml(say));
    if (play) {
        resp.play(play);
    } else {
 ...
 */
function buildVoiceGather({ say, action, timeout = 6, speechTimeout = 2, play = null }) {
    const resp = new VoiceResponse();
    const gather = resp.gather({
        input:          'speech dtmf',
        language:       LANGUAGE,
